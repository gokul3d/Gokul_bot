...
class NEURON: 
    ...
    def Update_Hidden_Or_Motor_Neuron(self, neurons, synapses):
        self.Set_Value(0.0)
        #print( f"Value of neuron {self.Get_Name()} before update: {self.Get_Value()}")  # Print the value of the neuron before the for loop

        for key in synapses:
            if key[1] == self.Get_Name():
                self.Allow_Presynaptic_Neuron_To_Influence_Me(synapses[key].Get_Weight(), neurons[key[0]].Get_Value())
        #print(f"Value of neuron {self.Get_Name()} after update: {self.Get_Value()}")
        self.Threshold()
    def Allow_Presynaptic_Neuron_To_Influence_Me(self, weight, prevalue):
        self.Add_To_Value(weight * prevalue)
